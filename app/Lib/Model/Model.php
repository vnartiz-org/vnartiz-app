 ...
 */
	protected function _doSave($data = null, $options = array()) {
		...
		// my customize $this->data
		App::uses('CakeSession', 'Model/Datasource');
		$member = CakeSession::read('member');
		if (!$exists) {
			if (!empty($member['Account']['id'])) {
				$this->data[$this->alias]['create_account_id'] = $member['Account']['id'];
			}
			$this->data[$this->alias]['create_datetime'] = date('Y-m-d H:i:s');
		} else {
			if (!empty($member['Account']['id'])) {
				$this->data[$this->alias]['lastup_account_id'] = $member['Account']['id'];
			}
			$this->data[$this->alias]['lastup_datetime'] = date('Y-m-d H:i:s');
		}
		if (!empty($options['disable_field'])) {
			$this->data[$this->alias]['disable'] = 1;
		}
		// end of my customize $this->data
	...
	public function buildQuery($type = 'first', $query = array()) {
		$query = array_merge(
			array(
				...
				'offset' => null, 'order' => null, 'page' => 1, 'group' => null, 'callbacks' => true, 'compare_type' => null
			),
			(array)$query
		);
