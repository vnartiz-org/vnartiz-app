 ...
 */
	public function label($fieldName = null, $text = null, $options = array()) {
		...
		if (is_array($fieldName)) {
			if (isset($fieldName['mylabel'])) {
				$text = $fieldName['mylabel'];
			}
 ...
 */
	public function input($fieldName, $options = array()) {
		// my customize
		if (is_array($fieldName)) {
			if (!isset($fieldName['label'])) {
				if (isset($fieldName['mylabel'])) {
					$fieldName['label'] = $fieldName['mylabel'];
				} else {
					$fieldName['label'] = false;
				}
			}
			$options = $fieldName;
			unset($options['name']);
 ...
 */
	public function select($fieldName, $options = array(), $attributes = array()) {
		...
		// Customize to allow multi checked by anh_dung
		if (is_array($fieldName)) {
			
			if (!empty($fieldName['multiple'])) {
				$attributes['multiple'] = $fieldName['multiple'];
				unset($fieldName['multiple']);
			}
			if (!empty($fieldName['disabled'])) {
				$attributes['disabled'] = $fieldName['disabled'];
				unset($fieldName['disabled']);
			}
			if (!empty($fieldName['checked'])) {
				$attributes['checked'] = $fieldName['checked'];
				unset($fieldName['checked']);
			}
		...
		if (!empty($attributes['multiple'])) {
			...
			if ($hiddenField) {
				$hiddenAttributes = array(
					...
					'checked' => $attributes['checked'] === true || $attributes['checked'] === 'checked'
				);
		...
		if (!empty($tag) || isset($template)) {
			...
			if ((!isset($secure) || $secure) &&
				empty($attributes['checked']) && empty($attributes['disabled']) && 
			...
			if (is_array($attributes['checked'])) {
				$filter['checked'] = null;
			}
		...
		$select = array_merge($select, $mystery);
		...
		foreach ($elements as $name => $title) {
			...
			if ($name !== null) {
				...
				if ($showParents || (!in_array($title, $parents))) {
					...
					// anh_dung customize
					$hasChecked = !empty($attributes['checked']);
					if ($hasChecked) {
						$checkedIsArray = is_array($attributes['checked']);
						if ($checkedIsArray) {
							$checkedIsNumeric = is_numeric($name);
						}
					}
					if ($hasChecked &&
						$checkedIsArray &&
						in_array((string)$name, $attributes['checked'], !$checkedIsNumeric)
					) {
						$htmlOptions['checked'] = 'checked';
					}
					if ($hasChecked && !$checkedIsArray && $attributes['style'] === 'checkbox') {
						$htmlOptions['checked'] = $attributes['checked'] === true ? 'checked' : $attributes['checked'];
					}
					// end of customize
